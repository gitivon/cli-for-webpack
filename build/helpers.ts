...
// HtmlWebpackPlugin
export const HtmlWebpackPluginHelper = (
  projectName: string,
  entry: WebpackEntry,
): HtmlWebpackPlugin[] => {
  ...
  // 获取 src 下的 js
  const options: HtmlWebpackPlugin.Options[] = Object.keys(entry).map(
    (chunk) => {
  ...
  return options.map((option) => new HtmlWebpackPlugin(option));
};
...
// 获取 webpack entry
export const getWebpackEntriesHelper = (
  projectName: string,
  pathname = 'src',
): WebpackEntry => {
  ...
  return dirFiles
    .filter(
      (file) => ['.ts', '.tsx', '.js', '.jsx'].indexOf(path.extname(file)) >= 0,
    )
    .reduce(
      (current, val, index) => ({
        ...current,
